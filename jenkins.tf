...
resource "null_resource" "jenkins_provisioner" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "jenkins_master ansible_host=${aws_instance.jenkins-server.public_ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/Documents/0712mac.pem" >> inventory.ini
      echo "" >> inventory.ini
      echo "[jenkins_slaves]" >> inventory.ini
    EOT
  }
}
*/